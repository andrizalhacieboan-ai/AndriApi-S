...
// In-memory SQLite fallback (local dev without Turso)
...
async function initDb() {
  ...
  // Auto-migration untuk kolom yang mungkin belum ada
  const migrations = [
  ...
  if ((planCount.rows[0]?.c || 0) === 0) {
    ...
    for (const p of plans) {
      await db.execute({
        sql: `INSERT OR IGNORE INTO plans (id,name,slug,price,request_limit_per_day,request_limit_per_hour,request_limit_per_minute,features,sort_order,is_active) VALUES (?,?,?,?,?,?,?,?,?,1)`,
        args: [p.id, p.name, p.slug, p.price, p.day, p.hour, p.min, JSON.stringify(p.feat), p.sort]
      });
    }
  }

  // Seed admin jika environment variable tersedia
