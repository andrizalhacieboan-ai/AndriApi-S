/**
 * Lokasi File: ./src/api/downloader/facebook.js
...
module.exports = function (app) {
  ...
  const handleFacebook = async (req, res) => {
    const postUrl = req.body.url || req.query.url;
    const cookie = req.body.cookie || req.query.cookie || "";
    const userAgent = req.body.useragent || req.query.useragent || null;

    if (!postUrl) {
      return res.status(400).json({
        status: false,
        statusCode: 400,
        message: 'Parameter "url" (Facebook video/post link) wajib diisi.',
        error: "URL_REQUIRED"
      });
    }
    ...
    try {
      ...
      const result = await downloader.download(postUrl);

      return res.status(200).json({
        status: true,
        statusCode: 200,
        message: "Success fetching Facebook media data",
        data: result
      });
    } catch (err) {
  ...
  app.get("/api/download/facebook", bypassOrCheckApiKey, handleFacebook);
  app.post("/api/download/facebook", bypassOrCheckApiKey, handleFacebook);
};
